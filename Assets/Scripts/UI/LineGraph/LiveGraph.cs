 ...
 * Updated:             2/14/2025
 * 
 * Creates a line graph that updates every tick to show the live stats.
 */
...
public class LiveGraph : MonoBehaviour
{
    ...
    public void UpdateLiveGraph()
    {
        foreach (LiveStat liveStat in _liveStats)
        {
            ...
            if (liveStat.Line.Points[liveStat.Line.Points.Count - 1].y < 0) //if y is less than the size of the grid,change it to be the bottom of the grid
                liveStat.Line.Points[liveStat.Line.Points.Count - 1] = new Vector2(liveStat.Line.Points[liveStat.Line.Points.Count - 1].x, 0);
